...
fileName = 'MER_23_45_LT_mergedData_v2.mat';
...
for f = 1:size(freqLims)
    ...
    for t = ind'
        ...
        allTimePoints = sort(allTimePoints)+1;
        ...
        for aa = 1:size(timeStamps{count},2)
            ...
            macroTrack{count}(1,aa) = peak2peak(centralSig(startPt:endPt));
            macroTrack{count}(2,aa) = peak2peak(posteriorSig(startPt:endPt));
            macroTrack{count}(3,aa) = peak2peak(medialSig(startPt:endPt));
    ...
    if ~isempty(firingDat)
        ...
        if ismember(depthLab,stimDepth)
            ...
            for ii = 1:size(firingDat,1)
                ...
                findpeakVals= [spikeProm 4*spikeThres 50
                    spikeProm 5*spikeThres 50
                    spikeProm*4 4*spikeThres 50];

                if ismember(count, [1 2 9 4 10 11 14 20 21])
                    param = 2;
