...
%                if ismember(count2,[3 4 5 8 9 10 13 14 15 16])
%                     if count==1
%                         figure(count2)
%             
...
% 3 4 5 8 9 10 13 14 15 16 ipsilateral leads
for tt = [1:16]%ceegInds'    
    count2 = count2+1;
    for ttt = 1:length(trackLabs)
    ...
    for tt = 1:size(meanPower{ff}{1,1},2) %compare tract
        ...
        for aa = windowsToPlot %compare windows
            ...
            for t = eegToPlot
                ...
                powerDif = mean(depth1Baseline)-mean(depth2Baseline);
...
%                     end
                    ...
                    sgtitle(['MER 39 RT ' windowLabs{aa}])
                    hold off
        ...
        if ismember(depthLab,stimDepth)
            ...
            for ii = 1:size(firingDat,1)
                ...
                findpeakVals= [spikeProm 5*spikeThres 50
                               spikeProm 3*spikeThres 50
                               spikeProm*1.5 5.0*spikeThres 50
                               100 100 100];
    
                if ismember(count, [10])
                    param = 2;
                elseif ismember(count, [1])
                    param = 3;
                elseif ismember(count, [2 5])
                    param = 4;
                end
%     
