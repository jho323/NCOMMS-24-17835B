    ...
    for t = ind'
        ...
        for tt = [1:16]%ceegInds'
            ...
            for aa = 1:size(timeStamps{count},2)
                ...
                %PRE
                [pxxPre, fPre, CIpre] = pwelch(preWindow,  window, noverlap, nfft, Fs,'ConfidenceLevel',0.95); % PSD estimate
...
%                 powerSpectraPre{f}{count2,count}{idx}{2} = CIpre;
                ...
                %STIM
                [pxxStim, fStim,CIstim] = pwelch(stimWindow, window, noverlap, nfft, Fs,'ConfidenceLevel',0.95); % PSD estimate
...
%                 powerSpectraStim{f}{count2,count}{idx}{2} = CIstim;
                ...
                %POST
                [pxxPost, fPost, CIpost] = pwelch(postWindow, window, noverlap, nfft, Fs,'ConfidenceLevel',0.95); % PSD estimate
...
%                 powerSpectraStim{f}{count2,count}{idx}{2} = CIpost;
...
%             ci_lower = (singleSpec{2}(:,1));
%             ci_upper = (singleSpec{2}(:,2));
