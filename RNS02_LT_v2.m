...
% 
%                     figure(t)
%                     subplot(length([1:4:16]),1,count2)
%                     hold on
%                     xline(startPoints,'--g')
%                     xline(endPoints,'--r')
%                     plot(time,cEEG)
...
%                 end
            ...
            for ttt = 1:length(trackLabs)
                preMean = mean(preWindowPower{f}{count2,count}{1},3);
...
%                 meanPower{f}{count2,count}{ttt} = [preComp(1:1000:end) stimComp(1:1000:end) postComp(1:1000:end)];

%                 meanPower{f}{count2,count}{ttt} = [preComp stimComp postComp];
...
%                 if ismember(count2,[1 2 3 6 7 8 11 12 13 16])
%                     if count==1
%                         figure(count2)
%             
%                         ax1 = subplot(3,size(freqLims,1),f+5*(ttt-1));
%                         boxplot(powerMat,p,'Labels',windowLabs,'symbol','+r')
%                         title([trackLabs{ttt} ' ' freqLabs{f}])
%                         sgtitle([convertStringsToChars(string(singleDat.depth)) ' ' convertStringsToChars(string(subFields{tt}))])
%                        
%                     else 
%                         figure(count2+20)
%             
%                         ax2 = subplot(3,size(freqLims,1),f+5*(ttt-1));
%                         boxplot(powerMat,p,'Labels',windowLabs,'symbol','+r')
%                         title([trackLabs{ttt} ' ' freqLabs{f}])
%                         sgtitle([convertStringsToChars(string(singleDat.depth)) ' ' convertStringsToChars(string(subFields{tt}))])
%                         
...
% ipsilateral leads: 1 2 3 6 7 8 11 12 13 16
for tt = [1:16]%ceegInds'    
    count2 = count2+1;
    for ttt = 1:length(trackLabs)
        ...
        for f = 1:size(freqLims)
             preComp = meanPower{f}{tt,depthSelect}{ttt,1};
             stimComp = meanPower{f}{tt,depthSelect}{ttt,2};
             postComp = meanPower{f}{tt,depthSelect}{ttt,3};
...
%              postComp = meanPower{f}{tt,depthSelect}{ttt,3};
             ...
             powerMat = [preComp;stimComp;postComp; powerMat];

             mult = 3*f-1;
%              p = [ones(size(preComp)); ones(size(stimComp))*2; ones(size(postComp))*3];
             p = [ones(size(preComp))+mult; ones(size(stimComp))*(2+mult); ones(size(postComp))*(3+mult); p];
        end
        figure(count2)
        
        ax1 = subplot(3,1,ttt);
%         boxplot(powerMat,p,'Labels',windowLabs,'symbol','+r')
        boxplot(powerMat,p,'symbol','+r')

        title([trackLabs{ttt} ': Delta, Theta, Alpha, Beta, Gamma'])
        sgtitle([depthLabs{depthSelect} ' ' convertStringsToChars(string(subFields{tt}))])
        axes(end+1) = ax1;
    ...
    for tt = 1:size(meanPower{ff}{1,1},2) %compare tract
        ...
        for aa = windowsToPlot %compare windows
            ...
            for t = eegToPlot
                ...
                depth1Baseline = meanPower{ff}{t,1}{tt,1};
                depth2Baseline = meanPower{ff}{t,2}{tt,1};
                powerDif = median(depth1Baseline)-median(depth2Baseline);
...
%                     end
                    ...
                    compDepthsVals(count3,ttt) = mean(singleTrial);
                end
...
%                     end
                    ...
                    sgtitle(['MER 39 LT ' windowLabs{aa}])
                    hold off
...
%                     sig = ttest(meanPower{ff}{t,trial1}{tt}(:,aa), meanPower{ff}{t,trial2}{tt}(:,aa),'Alpha',0.0001,'Tail','both');
    ...
    if ~isempty(firingDat)
        ...
        if ismember(depthLab,stimDepth)
            ...
            for ii = 1:size(firingDat,1)
                ...
                spikeThres = 4*std(filtDat);
                spikeProm = spikeThres;
                
                findpeakVals= [spikeProm spikeThres 5
                        2 .5 50
                        1 .4 50
                        4 1.9 50];
